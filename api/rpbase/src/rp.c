...
#include "rp_fpga_usr.h"  // user defined FPGA interface
...
int rp_Init()
{
    ...
    ECHECK(calib_Init());
    ECHECK(hk_Init());       // 0x00000000
    ECHECK(rp_usr_Init());   // 0x00600000 -- initialize the user defined interface
    ECHECK(ams_Init());      // 0x00400000
    ECHECK(generate_Init()); // 0x00200000
    ECHECK(osc_Init());      // 0x00100000
    //ECHECK(rp_usr_Init());   // 0x00600000
...
int rp_Release()
{
    ECHECK(osc_Release())       // 0x00100000
    ECHECK(generate_Release()); // 0x00200000
    ECHECK(ams_Release());      // 0x00400000
    ECHECK(rp_usr_Release());   // 0x00600000 -- release the user defined interface
    ECHECK(hk_Release());       // 0x00000000
    //ECHECK(rp_usr_Release());   // 0x00600000
    ECHECK(calib_Release());
    ECHECK(cmn_Release());
    //ECHECK(rp_usr_Release());
...
// HH 2017-02-18 added interfaces 

int rp_usr_SetPattern(uint32_t newval)
{
    /*uint32_t rv =*/ iowrite32(newval, &rp_usr->pattern);
    //printf("write pattern %X, %X\n",(unsigned)rv,(unsigned)&rp_usr->pattern); 
    return RP_OK;
}

int rp_usr_SetPeriodTime(uint32_t newval)
{
	iowrite32(newval, &rp_usr->period);
    return RP_OK;
}

int rp_usr_SetLed(uint32_t newVal)
{
    iowrite32(newVal,&rp_usr->led);
    return RP_OK;
}

// HH : no more changes below this comment
